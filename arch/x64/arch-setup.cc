...
void arch_setup_free_memory()
{
    ...
    // This will later patched W^X, unless runtime tracepoint adding feature (--runtime-tracepoint) is enabled.
    mmu::linear_map(elf_start, elf_phys_start, elf_size, OSV_KERNEL_BASE, mmu::mattr_default, mmu::perm_rwx);
...
/* Re-map kernel with proper perms.
 * This is done after mapping it once RWX, since we must map even before booting
 *  to run all kinds of initializers and argv parsing, which might require
 *  adding tracepoints.
 */
void arch_mprotect_kernel()
{
    // VA mappings are completed from arch_setup_free_memory,
    //  and PAs are now inaccessible. Use corresponding VA mappings.
    auto elf_header = static_cast<elf::Elf64_Ehdr*>(elf_start);
    static mmu::phys elf_virt_start = reinterpret_cast<mmu::phys>(elf_header);
    ...
    elf::Elf64_Shdr* shtbl = reinterpret_cast<elf::Elf64_Shdr*>(elf_virt_start + elf_header->e_shoff);
